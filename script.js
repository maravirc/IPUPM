...
function iniciarCabeceraFija() {
    ...
    window.addEventListener('resize', function() {
        ...
        // Ajustar fuente en resize
        const isMobile = window.innerWidth <= 600;
        const numeros = cabeceraFija.querySelectorAll('.numero');
        const titulos = cabeceraFija.querySelectorAll('.titulo');
        numeros.forEach(el => {
            el.style.fontSize = isMobile ? '14px' : '18px';
            el.style.padding = isMobile ? '2px 10px' : '4px 14px';
        });
        titulos.forEach(el => {
            el.style.fontSize = isMobile ? '14px' : '18px';
            el.style.maxWidth = isMobile ? '50vw' : '70vw';
        });
